...
The env var, not `-- --allow-default-credentials`: npm appends extra arguments to the end of
the script, and the script is a two-command chain, so the flag would land on vitest. Drop it
entirely if `NUXEO_USER` / `NUXEO_PASS` are not the Docker defaults. `NUXEO_URL` selects the
server and defaults to `http://localhost:8080`.
