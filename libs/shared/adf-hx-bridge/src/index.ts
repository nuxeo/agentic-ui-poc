...
// API ports, `provideAdfHxNuxeoBridge`, and adf-hx UI such as `HxpBrowseNavDrawerComponent`
// therefore live in `@agentic-ui/shared/adf-hx-bridge/providers`. Most of that surface is for
// the lazily-loaded POC route; the shell nav drawer and `app.config.ts` also import the
// providers entry on purpose (see `src/providers.ts` and the guardrail allowlist). Do not
// re-export those symbols here.
