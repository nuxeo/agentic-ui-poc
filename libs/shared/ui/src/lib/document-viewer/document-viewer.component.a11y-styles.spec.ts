...
describe('DocumentViewerComponent a11y styles (NXENG-901)', () => {
  ...
  it('styles format-type via the component light-surface token', () => {
    const scss = readFileSync(`${import.meta.dirname}/document-viewer.component.scss`, 'utf8');
    expect(scss).toContain('--document-viewer-muted-on-light-surface');
    const block = scss.match(/\.format-type\s*\{[^}]+\}/s)?.[0] ?? '';
    expect(block).toContain('var(--document-viewer-muted-on-light-surface)');
