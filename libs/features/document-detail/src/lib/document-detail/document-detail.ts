...
export class DocumentDetailComponent implements OnInit, OnDestroy {
  ...
  readonly documentCategory = computed(() => {
    ...
    if (!natureId) return '';
    const entry = this.natureVocabulary().find((item) => item.id === natureId);
   ...
   * Accepts id, display label (case-insensitive), and known KE alias labels.
   */
  private resolveNatureVocabularyId(value: string): string | null {
    return mapKeClassificationToNatureId(value, this.natureVocabulary());
  }
