...
| Recording | `NXENG-947-before.webm` (chapters in `chapters.vtt`) |

## Failed checks

- Scene 2 ([Act 2] Landmarks contain the sign-in panel) — **username label is inside a main landmark**: closest main: false
- Scene 2 ([Act 2] Landmarks contain the sign-in panel) — **copyright uses a footer landmark**: footer count: 0
...
### 2. Landmarks contain the sign-in panel  _(2 failed)_

_Screen reader users navigate by landmark; orphan labels fail WCAG 1.3.1 region_

Proves: **AC-1**

- [FAIL] username label is inside a main landmark — closest main: false
- [FAIL] copyright uses a footer landmark — footer count: 0
