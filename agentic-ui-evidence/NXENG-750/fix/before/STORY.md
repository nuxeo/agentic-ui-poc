...
**Verdict:** FAIL — 9/11 checks across 5 scene(s)
...
| Recording | `NXENG-750-before.webm` (chapters in `chapters.vtt`) |

## Failed checks

- Scene 2 ([Act 2] Keyboard focus on Log in (disabled, empty form)) — **disabled submit shows visible outline (2px+ solid) or focus shadow**: outline=none 3px, shadow=none
- Scene 4 ([Act 3] Login form fields still accept keyboard focus) — **Tab reaches username field**: focus on username input
...
### 2. Keyboard focus on Log in (disabled, empty form)  _(1 failed)_
...
- [FAIL] disabled submit shows visible outline (2px+ solid) or focus shadow — outline=none 3px, shadow=none
...
### 4. Login form fields still accept keyboard focus  _(1 failed)_

_Ensure focus styling changes did not break username/password tab stops_

Proves: **AC-3**

- [FAIL] Tab reaches username field — focus on username input
