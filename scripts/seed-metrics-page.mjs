...
const body = [
  p('Who ran the bug-fix skill, on which ticket, and how long it took. One row per run, appended automatically &mdash; nobody is expected to fill this in by hand.'),
  ...
  `<table><tbody>
    ...
    <tr><td><p><strong>Time taken</strong></p></td><td><p>Wall clock from the start of the run to the end, including time spent waiting on CI and on downloads.</p></td></tr>
  </tbody></table>`,

  h2('What is not here, and where it lives'),
  p('The <strong>per-phase breakdown</strong> &mdash; how long the reproduction took versus the fix versus getting the gate green, plus retries and gate failures &mdash; is deliberately not published. It is for tuning the skill rather than for reporting, and thirty columns produced a table nobody could read across. It is written to <code>~/Desktop/agentic-ui-evidence/&lt;TICKET&gt;/metrics.jsonl</code> on the machine that ran it, and printed by <code>node scripts/agent-metrics.mjs report &lt;TICKET&gt;</code>.'),
  ...
  h2('Runs'),
  `<table><tbody>
<tr>${th('User')}${th('Ticket ID')}${th('Time taken')}</tr>
</tbody></table>`,
].join('\n');
