...
const body = [
  p(
    'Who ran the bug-fix skill, on which ticket, and <strong>how long the fix itself took</strong>. One row per run, appended automatically &mdash; nobody is expected to fill this in by hand.',
  ),

  h2('How a row lands here'),
  p(
    '<strong>When the pull request is raised</strong> &mdash; not at the end of the run &mdash; the agent executes:',
  ),
  ...
  p(
    'It authenticates with the engineer&rsquo;s own <code>~/.jira_email</code> and <code>~/.jira_token</code>, so each row is written by the person who ran it and the page history shows who added what.',
  ),
  p(
    '<strong>Keep the &ldquo;Runs&rdquo; heading immediately above the table.</strong> The publisher finds the table by that heading. If the heading goes, it refuses to write rather than guessing and putting the row in the wrong table.',
  ),
...
const cur = await (
  await fetch(`${BASE}/api/v2/pages/${PAGE}`, {
