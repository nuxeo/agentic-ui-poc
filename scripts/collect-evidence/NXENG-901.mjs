...
export const scenes = [
  ...
  },
  {
    ...
    intent: 'Ensure the fix did not break keyboard focus on Preview download controls',
    criterion: 'AC-3',
    async run(page, h) {
      await openPreviewFormatType(page, h);
      ...
      if (downloadVisible) {
        ...
        const focused = await download.evaluate((el) => el === document.activeElement);
        h.check('download control accepts keyboard focus', focused, 'lib-document-viewer .format-download-btn');
        h.check(
          'download control remains visible',
          await download.isVisible(),
          'lib-document-viewer .format-download-btn',
        );
      } else {
        h.note(
          'No format download button on this document — AC-3 keyboard/download checks not applicable for this fixture',
        );
      }
      await h.expectNoConsoleErrors('preview tab', [
        /automation\/AI\./,
        '/nuxeo/logout',
