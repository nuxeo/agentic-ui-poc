...
| **Configuration survives upgrade**             | `install.xml` — app `overwrite="true"`, config sibling `overwrite="false"`; plus the rehearsal gate | Unchanged conclusion, different reason since 2026-09-26: **the ZIP now builds and publishes** (F13 withdrawn), but it **has never been installed on a server**, so the overwrite semantics are still read from `install.xml` rather than observed. The `upgrade-rehearsal` gate crosses a version boundary for the **npm package**, not the marketplace install. The mechanism is evidenced; the packaged install is not.                                                                                                                                           |
