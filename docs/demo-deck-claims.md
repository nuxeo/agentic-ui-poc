...
`docs/beta-demo-runbook.md` Part 6 lists fourteen things not to demo, dated 2026-08-25. **161
...
| **F5**  | **WRONG — withdrawn**       | Was true of the template it cites and is not true now. `nav-drawer.component.html:10` renders `{{ item \| descriptorLabel }}` and the rail `{{ navText(item) }}` (`app-shell.component.html:25`); both prefer the descriptor's `labelKey` and fall back to `label`. `AppTranslateLoader` layers the manifest's `labels` last over the catalogue (`app-translate-loader.ts:220`, pinned by its precedence spec). So `labels["nav.item.<slug>"]` **does** relabel navigation, per locale. `overrides.<id>.label` remains the way to set one literal for every locale, and wins when both are set. |
...
| **F13** | **WRONG — withdrawn**       | Re-checked 2026-09-26. The absent local `target/` said only that nobody had run Maven **on a laptop**; it runs in CI on every pull request. `2026.0.1-20260926071953-BUILD-1109` was built and published to preprod by run 36226401180 and is live on the listing. What is still unverified is **installing** the ZIP on a server, and then **upgrading over it** — two separate steps, and R7 turns on both.                                                                                                                                                                                   |

**Nine of the fourteen hold as written. Three were wrong and are withdrawn, one holds for a
different reason, one is unconfirmed.** That base rate is the argument for re-verifying rather than trusting a
dated document — F5 is the demonstration: it held when it was written, was re-verified as holding on
2026-09-01, and was withdrawn on 2026-09-26 when a reviewer read it against the template it cites
rather than against the earlier verdict.
...
| **The installer is configured to preserve configuration on upgrade** | `install.xml` — app `overwrite="true"`, config sibling `overwrite="false"`; plus the rehearsal gate | **Do not say "configuration survives upgrade"** — that asserts an outcome nobody has observed. What is evidenced is the installer's _configuration_: two copy steps, read from `install.xml`. Since 2026-09-26 the ZIP builds and publishes (F13 withdrawn), but it has never been installed, so neither the install nor an upgrade over it has been exercised. The `upgrade-rehearsal` gate does cross a version boundary — for the **npm package**, not the marketplace install. The mechanism is evidenced; the survival is not.                                 |
...
| **10 of 11 in-scope projects meet 90%**                              | `npm run beta:coverage -- --run`, 2026-09-22                                                        | **Was 10 of 10 until 2026-09-22 — do not present the older figure.** The denominator moved, not the code: `assets`, `tasks` and `shared-ai-client` had never been measured, and `shared-ai-client` came in at 15.98%, which reopened Phase 6 step 6. Also percentages of the **measured subset**: **6,137 in-scope lines are imported by no test at all**, contribute no statements, and cannot lower any percentage. Not the same claim as "the code is 90% tested".                                                                                               |
| **WCAG 2.1 AA met**                                                  | Phase 6 step 3 — **RE-VERIFY OR DROP THIS CLAIM**                                                   | **Do not present as-is.** Five consecutive `phase-6-a11y` captures since 2026-09-15 fail 3 checks — `button-name`, axe serious/critical, on browse, browse cards and the column panel. Undiagnosed as of 2026-09-23, so the claim is unverified rather than known false; the last passing capture is 2026-08-24. If it holds on a fresh run, the standing caveats still apply: 15 cases over 8 routes, **dialogs, upload, dark mode and the pre-auth login surface not covered**, and one upstream violation (`adf-core` `role="row"`) excluded on that step alone. |
...
| Do not say                                 | Why                                                                                                                                                                                            |
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~"The marketplace package builds"~~       | **Now sayable** (F13 withdrawn 2026-09-26): built and published to preprod, live on the listing. Do not extend it to "installs" or "upgrades" — neither has been done.                         |
