...
| WCAG 2.1 AA         | **RE-CHECK BEFORE QUOTING — the five most recent captures fail.** `phase-6-a11y` manifests from 2026-09-15T00-32-51 through 2026-09-16T08-51-23 each report `{steps:13, checks:27, failed:3}`: `button-name` (axe **serious/critical** — a button with no accessible name) on **browse**, **browse cards** and the **browse column panel**. The last passing capture is `2026-08-24T14-40-16`, 27/27. Not yet diagnosed, and not caused by the i18n work in PR #227, whose changes postdate these runs. Everything after this sentence describes that August pass: **met** on the 15 cases scanned — 7 rule classes fixed, `KNOWN_VIOLATIONS` empty. One violation remains and it is `@alfresco/adf-core`'s (finding 1.2), excluded on that one surface. Dialogs, upload, dark mode and pre-auth login are **not** covered |
...
| **`angular-oauth2-oidc` is kept**, unused; **`cropperjs` is used** | `angular-oauth2-oidc` causes no issue today, and unlike `pdfjs-dist` it does not tree-shake out. `cropperjs` was kept unused on the same basis until NXSAT-290, which made its stylesheet a global style, because adf-core's image viewer needs it. |
...
9. **The marketplace package has never been installed or upgraded on a real server**, so risk R7
   stays Medium. The outstanding exercise is a **marketplace install rehearsal** — not the
   `upgrade-rehearsal` gate, which is done and crosses a version boundary for the npm tarball
   (Step 1 above) and therefore cannot close R7. Narrowed 2026-09-26: it **has** now been
   built and published — `2026.0.1-20260926071953-BUILD-1109`, run 36226401180, live on the preprod
   listing — so the build and publish step is closed. **Two steps remain, not one:** a first
   install, and then an upgrade over it. Installing once would evidence the install and say
   nothing about upgrade, which is the step the configuration-survival story turns on.
...
| D-5 | **The marketplace package has never been installed or upgraded on a real server** — distinct from the completed npm `upgrade-rehearsal` gate. Built and published since 2026-09-26; **both the install and an upgrade over it** are what is left. | Run a marketplace install rehearsal — install, customise, upgrade over it — or accept the risk explicitly. The `upgrade-rehearsal` gate does not cover it.                                                                  | **Risk R7 stays Medium.** Phase 1 already shipped an install path that would have 404'd in every deployment; only a real install proves the corrected one.                                              | Eng lead              |
