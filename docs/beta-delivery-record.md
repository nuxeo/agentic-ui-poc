...
| 3 — adf-hx adoption                     | **in progress** — 12 ports bound, 5 components adopted | `phase-3-adf-hx` 49/49   |
...
2. **`sys_effectivePermissions` is hardcoded** to a minimal set for every document — now the last
   of the five recorded bridge defects, four of which are closed. The sort, the `totalCount` and the
   50-child ceiling were all fixed on 2026-08-22, after this file had wrongly reported them closed
   once already (§7).
