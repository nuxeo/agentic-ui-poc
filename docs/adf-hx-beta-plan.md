...
- `libs/shared/app-config` loads a **bootstrap file** pre-auth and a **runtime manifest** from the
  ...
  destructive copy — which is what the installer relies on to leave customer edits in place on
  upgrade. Intended effect of the copy layout; no marketplace upgrade has been run. `nuxeo.war` is the
...
| R7  | **The upgrade-safe installer path is prototyped but not rehearsed.** Phase 1 first installed the bootstrap file into `nxserver/web/…`, which nothing serves — it would have 404'd in every deployment. The path is corrected and the corrected destination is confirmed served, and since 2026-09-26 the package is built and published — `2026.0.1-20260926071953-BUILD-1109`, live on the preprod listing — but never installed or upgraded on a real server. Manifest-as-Nuxeo-document is genuinely evidenced.   | Medium             | Packaging sign-off on the second `install.xml` copy step, then a **marketplace install rehearsal** — first install, then an upgrade over it. Distinct from the `upgrade-rehearsal` gate, which is done and exercises the **npm tarball**; that gate cannot close R7. Only the marketplace exercise justifies Low.                                                    |
