...
(()=>{
    ...
    async function runTypeScriptGoShowConfig(executable, configFile) {
        const { stdout } = await (0, external_node_util_namespaceObject.promisify)(external_node_child_process_namespaceObject.execFile)(executable.command, [
            ...executable.args,
            '--project',
            configFile,
            '--showConfig',
            ...
            cwd: external_node_path_default().dirname(configFile),
            encoding: 'utf8',
            maxBuffer: 67108864
        });
        const parsedConfig = JSON.parse(stdout);
        if (!parsedConfig || 'object' != typeof parsedConfig || Array.isArray(parsedConfig)) throw new TypeError('Invalid tsgo --showConfig output.');
        return parsedConfig;
    }
    function toComparisonPath(file) {
        const normalizedPath = external_node_path_default().normalize(file);
        return 'win32' === process.platform ? normalizedPath.toLowerCase() : normalizedPath;
    }
    function shouldRefreshTypeScriptGoDependencies(dependencies, { changedFiles = [], deletedFiles = [] }) {
        if (!dependencies) return true;
        const dependencyKeys = dependencies.files.map(toComparisonPath);
        return changedFiles.some((file)=>{
            const key = toComparisonPath(file);
            const extension = external_node_path_default().extname(file).toLowerCase();
            return '.json' === extension || dependencies.extensions.includes(extension) && !dependencyKeys.includes(key);
    ...
    async function getTypeScriptGoDependencies(config) {
        const executable = await resolveTypeScriptGoExecutable(config);
        const files = new Set();
        const dirs = new Set();
        const excluded = new Set();
        const visitedConfigs = new Set();
        const collectConfigDependencies = async (configFile, fallbackRoot, collectExcluded)=>{
            const normalizedConfigFile = external_node_path_default().normalize(configFile);
            const configKey = toComparisonPath(normalizedConfigFile);
            if (visitedConfigs.has(configKey)) return;
            visitedConfigs.add(configKey);
            const configDir = external_node_path_default().dirname(normalizedConfigFile);
            const normalizedRoot = external_node_path_default().normalize(fallbackRoot);
            files.add(normalizedConfigFile);
            if (!Array.from(dirs).some((dir)=>'' === external_node_path_default().relative(dir, normalizedRoot) || isInsideAnotherPath(dir, normalizedRoot))) dirs.add(normalizedRoot);
            excluded.add(external_node_path_default().join(normalizedRoot, 'node_modules'));
            let parsedConfig;
            try {
            ...
            for (const file of parsedConfig.files || [])files.add(external_node_path_default().normalize(external_node_path_default().resolve(configDir, file)));
            if (collectExcluded && !parsedConfig.references?.length) for (const exclude of parsedConfig.exclude || [])excluded.add(external_node_path_default().normalize(external_node_path_default().resolve(configDir, exclude)));
            for (const reference of parsedConfig.references || [])if (reference.path) {
                const resolvedReference = external_node_path_default().resolve(configDir, reference.path);
                const referenceConfig = '.json' === external_node_path_default().extname(resolvedReference).toLowerCase() ? resolvedReference : external_node_path_default().join(resolvedReference, 'tsconfig.json');
                await collectConfigDependencies(referenceConfig, external_node_path_default().dirname(referenceConfig), false);
            }
        };
    ...
    function tapStartToRunTypeScriptGo(compiler, config, state) {
        ...
        compiler.hooks.compilation.tap('TsCheckerRspackPlugin', async (compilation)=>{
            ...
            state.dependenciesPromise = startPromise.then((nextFilesChange)=>{
                if (cacheGeneration !== dependenciesCacheGeneration) return;
                if (state.watching && dependenciesPromise && !shouldRefreshTypeScriptGoDependencies(state.lastDependencies, nextFilesChange)) {
                ...
                if (state.watching) {
                    ...
                    nextDependenciesPromise.catch(()=>{
                        if (dependenciesPromise === nextDependenciesPromise) dependenciesPromise = void 0;
                    });
                }
