...
describe('LoginPageComponent', () => {
  ...
  it('keeps the password field above siblings when focused (NXENG-749)', () => {
    ...
    const passwordField = passwordInput.closest('mat-form-field') as HTMLElement;
    const wrapper = passwordInput.closest('.mat-mdc-text-field-wrapper') as HTMLElement;
    expect(passwordField).toBeTruthy();
    expect(passwordField.classList.contains('login-field-password')).toBe(true);
    expect(getComputedStyle(wrapper).overflow).toBe('visible');
    expect(getComputedStyle(passwordField).zIndex).toBe('2');
    expect(getComputedStyle(passwordField).position).toBe('relative');
    expect(getComputedStyle(passwordInput).scrollMarginBlock).not.toBe('0px');
  });
