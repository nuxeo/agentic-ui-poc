...
getBrowseFolderContents(nuxeoPath: string, pageSize?: number, options?: BrowseFolderLoadOptions): Observable<BrowseFolderContents>  // browse page loader with restricted-user fallbacks; set `bootstrapOnRootDenied: false` to surface root 403 instead of synthetic root
