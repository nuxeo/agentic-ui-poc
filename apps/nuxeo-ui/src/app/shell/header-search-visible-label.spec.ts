...
describe('AppShellComponent — header global search visible label (NXENG-798)', () => {
  ...
  beforeEach(async () => {
    ...
    transitionSuppress.textContent = '.header-search-label { transition: none !important; }';
  ...
  it('keeps a shrunken visible label while a query is entered and restores the empty state when cleared', fakeAsync(() => {
    ...
    expect(Number.parseFloat(filledLabelStyle.fontSize))
      .withContext('caption must shrink the live AppShell label (styles.scss single declaration)')
    ...
    expect(filledLabelStyle.display)
      .withContext('caption must not be display:none while a query is entered')

      .not.toBe('none');

    expect(filledLabelStyle.visibility)
      .withContext('caption must not be visibility:hidden while a query is entered')

      .not.toBe('hidden');

    const { width, height } = filledLabel.getBoundingClientRect();

    expect(width)
      .withContext('caption must occupy horizontal space while a query is entered')

      .toBeGreaterThan(0);

    expect(height)
      .withContext('caption must occupy vertical space while a query is entered')
